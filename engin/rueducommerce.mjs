...
const extractModelAndClock = (inputStrings) => {
  ...
  if (inputString.includes("ryzen")) {
    ...
    let match = inputString.match(
      /ryzen™ 9\s+(\d+X)\s*-\s*(\d+\.\d+)\s*\/\s*\d+\.\d+\s*ghz/i
    );
    if (match) {
      model = "7900X";
      clock = match[2];
    ...
    let match1 = inputString.match(
      /i\d+-\w+\s*\((\d+\.\d+)\ GHz\s*\/\s*(\d+\.\d+)/
    );
    if (match1) {
      ...
      let match2 = inputString.match(
        /i\d+-\w+\s*-\s*(\d+\.\d+)\/\d+\.\d+\ gHz/
      );
      if (match2) {
        model = inputString.match(/i\d+-\w+/)[0];
        clock = match2[1];
...
const extractProcessorInfo = (inputStrings) => {
  ...
  if (inputString.includes("cores")) {
    matchA = inputString.match(
      /(\d+)\s*cores?\/(\d+)\s*threads? - socket (intel|amd)?\s*(\w+)/i
    );
    matchB = inputString.match(
      /(\d+)\s*cores?\/(\d+)\s*threads? - socket (intel|amd)?\s*(\w+)/i
    );
    matchC = inputString.match(
      /(\d+)\s*cores?\/(\d+)\s*threads? - socket (intel|amd)?\s*(\w+)/i
    );
  } else {
    matchA = inputString.match(
      /(\d+)\s*cœurs?\/(\d+)\s*threads? - socket (intel|amd)?\s*(\w+)/i
    );
    matchB = inputString.match(
      /(\d+)\s*cœurs?\/(\d+)\s*threads? - socket (intel|amd)?\s*(\w+)/i
    );
  ...
  $("#listing-infinite")
    ...
    .each((index, element) => {
      ...
      if (x.title.includes("Ryzen")) {
        if (x.Model == "") {
          //Ryzen™ 9 7900X - 4.7/5.6 Ghz
          const regex =
            /Ryzen™.*?\s(?:Threadripper™\s)?(\d+[X]?)\s+-\s+(\d[\d.,]+)\s*\/.*?(\d[\d.,]+)\s*GHz/;
        ...
        if (x.Model == "") {
          ...
          const startIndex =
            nameVal[5].trim().indexOf("Ryzen") + "Ryzen".length;
        ...
        if (x.Model == "") {
          ///Intel® Core™ i5-11400 - 2,6/4,4 GHz
        ...
        if (x.Model == "") {
          const regex =
            /Intel Core i\d+\s?-?\s?(\d+-?\d*[TF]?)\s+(\d+(\.\d+)?)\s*GHz/;
          // Extract model and clock speeds using the regex pattern
          const match = x.title.match(regex);
...
const fillMPN = async (x) => {
  ...
  let existing = await CPUInfo.findOne({
    Manufacturer: x.Manufacturer.toUpperCase(),
