      ...
      for (const element of priceElements) {
        try {
          ...
          for (const info of listinfo) {
            try {
                let button = await driver.findElements(By.className('uc-accept-button'));

                if (button.length > 0) {
                console.log("Button with class 'uc-accept-button' exists on the webpage.");
                } else {
                console.log("Button with class 'uc-accept-button' does not exist on the webpage.");
                }

                
              await driver
                .findElement(By.className("productOffers-listLoadMore"))
                .click();
